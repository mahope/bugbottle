...
`fileStore` has a ceiling, but it is a disk ceiling rather than a policy:
`maxReports` (2000 by default) deletes the oldest files once the directory is
over it, and `0` keeps everything. Age is not part of it. If your answer is
"delete after 90 days", write it — `reports.list()` gives you `receivedAt` for
every report, and `reports.remove(id)` deletes the JSON and the picture
together. A database `store` is the same job with a `DELETE`.
