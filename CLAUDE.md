...
| `site/da/privatliv/` | **Generated, never committed**, by the same run, from `site/da/privatliv.md`: the Danish half of the README's `## A privacy checklist`. Unlike "Kom i gang" it *is* the same page as its English twin at `/docs/privacy-checklist/` — same fields, same order — so the `STANDALONE` entry names an `otherUrl` and `TRANSLATED` in the script gives the English page its half of the `hreflang` pair. `SLUG_OVERRIDES` shortens the README's own `a-privacy-checklist` to that URL, keeping the GitHub anchor intact. `tests/privacy-checklist.test.ts` pins both tables to the properties of the generated report schema | site/da/privatliv.md (at image build) |
| `src/server/` | Re-exports of report-core, markdown and the sinks for `bugbottle/server` | report-core, markdown, sinks |
| `src/server/handle.ts` | `handleReport(request, options)` — `Request` in, `Response` out: 405 for anything but POST, authorise, body cap and body deadline, the optional HMAC `signature` check over the raw text, every validator, `extra`, scrub, screenshot policy, `store`, ordered sinks under a per-sink deadline. Plus `ValidatedReport` and the `toResend`/`toWebhook`/`toGithub`/`toLinear` sink helpers. The replay cache is bucketed by the *signed second* and bounded inside each one (128 digests, 640 seconds), because the signing key is public: a flood of valid signatures must not be able to evict an honest digest dated any other second. `signature.store` (`has`/`add(digest, expiresAt)`) replaces it with a shared one; `t=` is `/^\d{1,16}$/` and the digest is verified over the timestamp as it was sent. The same seam twice more, for the fleet that wants one answer: `rateLimit.store` (`hit(key, windowMs)` → the count) and `dedupe.store` (`get`/`set(key, entry, expiresAt)`, expiry the store's job). Both check what the store answered with before believing it — a finite number from `hit`, an entry shape from `get` — because a raw `"3"` or a bare string compared with `>` or read as an entry would switch the limit off, or make every report a duplicate, without a word. Both fail **open** through `onError` — an honest report is never refused because a shared store blinked — where the replay store fails closed. The rate limit keys on `clientAddress(request, remoteAddress, trustProxy)` and nothing else: the connection address the runtime handed in, unless `trustProxy` says a forwarding header may name the caller — `true` the last `X-Forwarded-For` entry, `{ hops: n }` n in from the right, `{ header }` a platform's own. It defaults to `false` because reading a header unconditionally is not a limit, and a chain shorter than `hops` falls back to the connection rather than reaching left towards the caller. `onDecision` is called once per request with `{ id?, status, reason, address, fingerprint?, at }` — every `respond` in the function goes through one `decide`, which is what stops a status added later from skipping the hook, and the CORS preflight is the one request it says nothing about; the eleven reason words are `stored`, `accepted`, `duplicate`, `not-post`, `rate-limited`, `unauthorised`, `too-large`, `timeout`, `bad-signature`, `invalid` and `error`, a custom `respond`'s status is read back off the response it built, the address is the `trustProxy` one, and the decision carries the fingerprint but nothing out of the report. A hook that throws reaches `onError` and changes no answer. Dedupe keys on the report's fingerprint and is not affected | report-core, markdown, scrub, sinks |
| `src/server/file-store.ts` | `fileStore({ dir, maxReports?, maxAgeDays?, screenshots? })` — the `store` function `handleReport` takes, backed by a directory, plus `list()`, `read(id, { screenshot? })`, `remove(id)`, `prune()` and `refresh()`. `maxAgeDays` is retention and `prune()` is what applies it — age first, then the cap, answering with how many reports went; off by default and scheduled by nobody, so `examples/inbox` runs it at start and hourly behind `RETENTION_DAYS`. It is safe beside a concurrent `store` (the same in-place splice the cap uses), walks the directory on its first call so an earlier run's reports are pruned too, leaves an unparseable `receivedAt` for the cap rather than guessing at its age, and deletes nothing whose name is not `<time>-<id>.json` for its own id — a file the store did not write is never touched. One JSON file per report named `<receivedAt>-<id>.json` with the decoded PNG beside it as `<id>.png`, an index built by one walk on the first call that needs it — one walk, shared by every caller that arrives before it finishes, and never replaced afterwards, only added to and spliced from, because a write holds it across an `await` — and kept up to date by every write and delete, and `maxReports` deleting the oldest first. An entry `read` finds nothing behind is dropped from the index there and then, because a listing that lies is a link answering 404 for the rest of the run; `refresh()` is the deliberate re-walk, splicing the same array rather than replacing it, for a directory something else has written to. The only module under `src/server/` that reaches for Node, and nothing the validators reach imports it, so the validator-only bundle still mentions neither `node:fs` nor `readFile`. Every write is a tmp file renamed into place, so a crash leaves a `.tmp` nothing lists rather than half a report; an id is matched against the `randomUUID` shape **before** a path is built, and the `receivedAt` that is the other half of the name keeps only the characters a timestamp is made of, which together are the whole traversal defence; a picture is signature-checked in its bytes before it is written under a `.png` name, and a failed one is dropped while the report is stored | report-core, handle (types only) |
| `src/server/express.ts` | `expressHandler(options)` — builds a web `Request` from an Express `req` and writes the `Response` back, counting and streaming-decoding a raw body itself. Structural types, no `@types/express`. A signed route mounted behind `express.json()` cannot be verified at all, so it calls `onError` once per handler naming the parser and answers the same 401. It passes `req.socket.remoteAddress` (then `req.ip`) in as `remoteAddress`, so `trustProxy` is the one setting deciding what the rate limit counts — reading `req.ip` first would trust Express's own `trust proxy` behind our back | server/handle |
...
| `scripts/a11y-audit.mjs` | Serves `dist/` on a scratch page, mounts the panel and runs the pinned `axe-core` over ten states through `puppeteer-core`, two of them with the forced-colours palette and one with `prefers-reduced-motion: reduce`, then photographs the forced-colours one and reads pixels out of the picture, and reads the computed durations out of the reduced-motion one. The first half of `npm run a11y`; not part of `npm run check`, because it needs a browser, but CI's `browser` job runs it on every push and pull request | dist (at run time), chrome |
| `scripts/a11y-site.mjs` | The same audit aimed at the pages rather than the widget: both landing pages, the documentation index, one deep documentation page, the search field on results, the theme playground with four controls moved, the two comparison pages, the Danish getting-started page, the changelog, and the landing and deep documentation pages again under `prefers-reduced-motion: reduce`, in both colour schemes — thirty runs — failing on a console message as well as on a violation. The second half of `npm run a11y`; needs `npm run build:docs` first, and runs in CI's `browser` job beside the panel audit | site, dist (at run time), chrome |
| `scripts/motionless.mjs` | `movingElements(tab)` + `reportMotion(label, moving)` — the reduced-motion check both audits make, in one place because both ask the same question. Walks the document and every open shadow root and returns a line per element whose `transition-duration`, or a running `animation-duration`, is above zero, plus one for a document that still scrolls smoothly. axe has no rule for motion, so this is the only thing that would notice | nothing (runs inside the page) |
...
- **No new control in `src/ui/` without a label and a locale string.** Every
  ...
  `node scripts/a11y-audit.mjs` (zero axe violations, ten states).
- **Nothing in the panel may move under `prefers-reduced-motion: reduce`.**
  The state changes stay — the panel opens, the picture attaches, the status is
  announced — they just happen at once. A transition or animation added to
  `src/ui/` is therefore also added to the media block at the top of `CSS`, and
  a scroll a script asks for reads the media query itself, because no
...
npx --yes @redocly/cli@2.51.2 lint dist/openapi.json  # zero errors, zero warnings; needs a build first
                    # pinned, and run by CI's `check` job straight after the build
...
`bugbottle/queue-idb` is budgeted at 1024 bytes and measures 661: one open, one
...
#88, the review after 0.13, moved nothing out of a budget: `bugbottle/queue`
1553 → 1545 (a method left the seam), `bugbottle/queue-idb` 654 → 661
(`versionchange`), `bugbottle/ui` 11 485 → 11 486, `dist/bugbottle.js`
24 424 → 24 441 and `dist/bugbottle.slim.js` 20 886 → 20 896. The
validator-only server bundle stayed at 584 bytes, which is what says the
sink fixes reached no shared path. #90 left it there too — 584 gzipped over
the same 1025 minified bytes — because `trustProxy` and `clientAddress` live
on the `handleReport` path a validator-only consumer never reaches. #89 left it there as well: retention lives in
`file-store.ts`, which no validator reaches.
...
and the core +3, which is the compressor rather than the code. #98 then cost
the queue 26 bytes (1539 → 1565) and the two script-tag builds about fifty
each (24 545 → 24 596 and 20 992 → 21 042) for the signer the auto-mount now
hands the queue; the budgets stay at 1600, 25088 and 21504. Measure the core
...
ten states (closed, open light, open dark, annotator light, annotator dark,
contact light, contact dark, the panel and the annotator again under
`forced-colors: active`, and the panel under
...
focus ring differs from the panel behind it. The reduced-motion state ends the
same way, for the same reason: a transition is not in the accessibility tree,
so the run reads every computed `transition-duration` and
`animation-duration` out of the shadow root and fails on any above zero.
`scripts/a11y-site.mjs` is the same procedure
aimed at `site/`: fifteen pages and states in two colour schemes, the last two
of them the reduced-motion check over the landing and documentation pages, and
a console message counts
