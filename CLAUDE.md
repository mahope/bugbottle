...
| `src/sinks/` | Server-only delivery: `sendReportEmail` (Resend), `sendReportWebhook` (json/slack/discord), `createGithubIssue`, `createLinearIssue` (GraphQL, so a rejected mutation arrives as a 200 with `errors` and still throws), `jiraSink`, `gitlabSink`, `sentrySink`, `teamsSink` (all below), the shared `SinkError`. One `fetch` each, keys and URLs are arguments — never `process.env` | markdown, locales, report-core |
...
with the Sentry, Jira, GitLab and Teams sinks exported, which is the proof that no
