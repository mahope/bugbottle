...
export function collectContext(): ReportContext {
  ...
  if (nav.language) context.language = nav.language;
  try {
    const zone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    if (zone) context.timezone = zone;
  } catch {
    // A runtime without a full ICU build has no zone to give.
  }
  const screen = window.screen;
  if (screen) context.screen = `${screen.width}x${screen.height}@${window.devicePixelRatio ?? 1}`;
  try {
  ...
  if (typeof nav.onLine === "boolean") context.online = nav.onLine;
  const connection = (nav as { connection?: { effectiveType?: unknown } }).connection;
