...
/** `DeviceMotionEvent` as Safari extends it. Everywhere else the method is absent. */
type MotionEventConstructor = {
...
export async function requestShakePermission(): Promise<"granted" | "denied" | "unsupported"> {
  const motion = (globalThis as { DeviceMotionEvent?: MotionEventConstructor }).DeviceMotionEvent;
  if (typeof motion?.requestPermission !== "function") return "unsupported";
  try {
 ...
 * ```
 */
export function onShake(handler: () => void, options: ShakeOptions = {}): () => void {
  const found =
    options.target ?? (typeof window === "undefined" ? undefined : (window as ListenerHost));
  if (!found) return () => {};
  const host: ListenerHost = found;
  ...
  const listener = (event: Event): void => {
    const reading = (event as unknown as ShakeEvent).accelerationIncludingGravity;
    if (!reading) return;
    ...
    let moved = 0;
    for (let i = 0; i < 3; i++) {
      const raw = axes[i] as number;
      const settled = (gravity[i] as number) * GRAVITY_SMOOTHING + raw * (1 - GRAVITY_SMOOTHING);
      gravity[i] = settled;
      ...
      if (Math.abs(high) > Math.abs(moved)) moved = high;
    }
    ...
    if (crossings.length < REQUIRED_CROSSINGS || now < quietUntil) return;
  ...
  function listen(on: boolean): void {
    ...
    listening = on;
    if (on) host.addEventListener("devicemotion", listener);
    else {
  ...
  const onVisibility = (): void => listen(doc?.visibilityState !== "hidden");
