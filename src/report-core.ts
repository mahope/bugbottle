...
export function normaliseStorage(raw: unknown): StorageSnapshot | null {
  ...
  if (typeof o.values === "object" && o.values !== null && !Array.isArray(o.values)) {
    ...
    for (const [key, value] of Object.entries(o.values as Record<string, unknown>)) {
      ...
      defineOwn(
        values,
        stripNullBytes(key).slice(0, MAX_STORAGE_KEY_LENGTH),
        stripNullBytes(value).slice(0, MAX_STORAGE_VALUE_LENGTH),
      );
    }
 ...
 */
function stripNuls(value: unknown, depth: number): unknown {
  if (depth > MAX_REPLAY_DEPTH) throw new ReplayTooDeepError();
  ...
  if (Array.isArray(value)) return value.map((item) => stripNuls(item, depth + 1));
...
export function normaliseReplay(raw: unknown): ReplayCapture | null {
  ...
  if (serialised.includes(NUL_ESCAPE)) {
    try {
      clean = stripNuls(events, 0) as ReplayEvent[];
    } catch {
