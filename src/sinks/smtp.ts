 ...
 */
function encodedWordBytes(limit: number): number {
 ...
 */
function encodeWords(value: string, limit = MAX_ENCODED_WORD): string {
 ...
 */
function encodeAddress(address: string, limit: number): string {
  const angled = /^(.*?)\s*(<[^>]*>)$/.exec(address);
  if (!angled) return needsEncoding(address) ? encodeWords(address, limit) : address;
  const name = angled[1] ?? "";
  const bare = angled[2] ?? "";
  if (!name) return bare;
