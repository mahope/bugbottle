 ...
 */
export function jiraAuthHeader(email: string, apiToken: string): string {
...
function paragraph(text: string): AdfNode | undefined {
  ...
  if (content.every((node) => node.type === "hardBreak")) return undefined;
  return { type: "paragraph", content };
}

/** One bullet per fact. A `listItem` must hold a block, so each holds a paragraph. */
function bulletList(lines: string[]): AdfNode | undefined {
  const items = lines
...
function codeBlock(text: string, language: string): AdfNode | undefined {
  if (!text) return undefined;
...
export function buildJiraDescription(
  ...
  options: Pick<
    JiraSinkOptions,
    "facts" | "maxConsoleEntries" | "screenshotUrl" | "screenshotUrlFrom"
  > = {},
  ctx: ChatSinkContext = {},
): AdfDoc {
  ...
  if (screenshot) facts.push(`Screenshot: ${screenshot}`);

  for (const [key, value] of Object.entries(options.facts ?? {})) {
    if (value !== undefined && value !== "") facts.push(`${key}: ${value}`);
  }

  const content: AdfNode[] = [];
  const words = paragraph(message);
  if (words) content.push(words);
  ...
  if (elements.length > 0) {
    const heading = paragraph(
      elements.length > 1 ? "Elements pointed at" : "Element pointed at",
    );
    ...
    if (elementList) content.push(elementList);
  }

  if (entries.length > 0) {
    const heading = paragraph(
      `Console (${entries.length} ${entries.length === 1 ? "entry" : "entries"})`,
    );
    if (heading) content.push(heading);
    const text = entries
...
function summaryOf(report: unknown): string {
  const r = (typeof report === "object" && report !== null ? report : {}) as Record<
    string,
    unknown
  >;
  ...
  return clip(
    `${TYPE_LABEL[type]}: ${clip(firstLine, MAX_TITLE_LENGTH) || TYPE_LABEL[type]}`,
    MAX_JIRA_SUMMARY,
  );
}
 ...
 */
export function messageFromJiraBody(body: unknown, fallback: string): string {
  if (typeof body !== "object" || body === null) return messageFromBody(body, fallback);
 ...
 * ```
 */
export function jiraSink(options: JiraSinkOptions): JiraSink {
  ...
  return async (report, ctx = {}) => {
    ...
    const issue = (body ?? {}) as { id?: unknown; key?: unknown; self?: unknown };
    return {
