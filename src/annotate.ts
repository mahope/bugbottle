...
type Shape = { tool: AnnotateTool; x: number; y: number; w: number; h: number };
...
export function createAnnotator(
  ...
  if (!context) throw new Error("createAnnotator needs a canvas with a 2D context");
  ...
  let tool: AnnotateTool = options.tool ?? "rect";
  ...
  function pixelate(x: number, y: number, w: number, h: number): void {
  ...
  function redraw(): void {
    ...
    for (const shape of shapes) paint(shape);
  ...
  return {
    ...
    destroy() {
      ...
      canvas.removeEventListener("keydown", onKey as EventListener);
    },
  };
}
