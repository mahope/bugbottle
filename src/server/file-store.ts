...
export function fileStore(options: FileStoreOptions): FileStore {
   ...
   */
  let index: StoredReport[] | null = null;
   ...
   */
  let walking: Promise<StoredReport[]> | null = null;

  function ensureIndex(): Promise<StoredReport[]> {
  ...
  async function readDirectory(): Promise<StoredReport[]> {
    ...
    for (const file of files.filter((name) => name.endsWith(".json")).sort().reverse()) {
      ...
      if (!id) continue;
      try {
        const report = JSON.parse(await readFile(join(dir, file), "utf8")) as ValidatedReport;
   ...
   */
  async function pruneToCap(): Promise<number> {
    if (!index || !Number.isFinite(maxReports) || maxReports <= 0) return 0;
    let deleted = 0;
   ...
   */
  async function pruneToAge(): Promise<number> {
    if (!index || !Number.isFinite(maxAgeDays) || maxAgeDays <= 0) return 0;
    const cutoff = Date.now() - maxAgeDays * DAY_MS;
    let deleted = 0;
    for (const entry of [...index]) {
      const arrived = Date.parse(entry.receivedAt);
      if (!Number.isFinite(arrived) || arrived > cutoff) continue;
   ...
   */
  function ownFile(entry: StoredReport): boolean {
    if (!UUID.test(entry.id)) return false;
  ...
  const store: FileStore["store"] = async (report, screenshot) => {
    ...
    if (picture) await writeAtomic(join(dir, `${id}.png`), picture);
 ...
 */
function nameSafe(receivedAt: string): string {
...
function summarise(
  id: string,
  file: string,
  report: ValidatedReport | null | undefined,
  screenshot: boolean,
): StoredReport {
  return {
    id,
 ...
 */
function pngBytes(screenshot: Uint8Array | string | undefined): Uint8Array | undefined {
  if (!screenshot || screenshot.length === 0) return undefined;
  if (typeof screenshot === "string") {
    try {
      return decodeScreenshotDataUrl(screenshot);
    } catch (error) {
      if (error instanceof InvalidScreenshotError) return undefined;
      throw error;
    }
  }
  if (screenshot.length > MAX_SCREENSHOT_BYTES) return undefined;
  if (screenshot.length < PNG_SIGNATURE.length) return undefined;
  for (let i = 0; i < PNG_SIGNATURE.length; i++) {
    if (screenshot[i] !== PNG_SIGNATURE[i]) return undefined;
  }
  return screenshot;
}
