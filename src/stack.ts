...
import { MAX_STACK_FRAMES, MAX_STACK_STRING_LENGTH, type StackFrame } from "./report-core.ts";
 ...
 */
export function parseStack(stack: unknown, maxFrames: number = MAX_STACK_FRAMES): StackFrame[] {
  ...
  for (const line of stack.split("\n")) {
    ...
    if (fn) frame.fn = fn.slice(0, MAX_STACK_STRING_LENGTH);
    frames.push(frame);
  }
  return frames;
}
