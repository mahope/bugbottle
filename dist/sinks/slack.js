...
function mrkdwn(text, max = MAX_SLACK_TEXT) {
...
export function buildSlackMessage(report, options, ctx = {}) {
    ...
    if (r.facts.length > 0) {
        blocks.push({
            ...
            fields: r.facts
                ...
                .map(([label, value]) => mrkdwn(`*${escapeSlack(label)}*\n${escapeSlack(value)}`, MAX_SLACK_FIELD_TEXT)),
        });
    }
    if (r.consoleText) {
    ...
    const context = [];
    if (r.timestamp)
        context.push(r.timestamp);
    if (r.selector)
        context.push(`\`${escapeSlack(r.selector)}\``);
    if (context.length > 0) {
 ...
 * ```
 */
export function slackSink(options) {
