 ...
 */
export const SENTRY_CLIENT_VERSION = "0.15.0";
...
export function parseSentryDsn(dsn) {
    ...
    if (!publicKey)
        throw new TypeError("The Sentry DSN has no public key");
    ...
    if (!projectId)
        throw new TypeError("The Sentry DSN has no project id");
...
export function sentryAuthHeader(publicKey, client = SENTRY_CLIENT) {
...
function seconds(iso, fallback) {
    if (!iso)
        return fallback;
    const ms = Date.parse(iso);
    return Number.isNaN(ms) ? fallback : ms / 1000;
}
function consoleCrumb(entry, fallbackTs) {
    return {
...
function activityCrumb(crumb, fallbackTs) {
    ...
    const out = { timestamp, type: "default", category: "ui.visibility" };
    if (crumb.to)
        out.message = crumb.to;
    return out;
}
function networkCrumb(entry, fallbackTs) {
    const data = {
        url: entry.url,
        method: entry.method,
 ...
 */
export function buildSentryEvent(report, options, ctx = {}) {
    const raw = (typeof report === "object" && report !== null ? report : {});
    ...
    const tags = { type };
    if (context.url)
        tags.url = context.url;
    if (context.viewport)
        tags.viewport = context.viewport;
    ...
    if (contactName)
        feedback.name = contactName;
    const associated = options.associatedEventId?.(report);
    if (associated)
        feedback.associated_event_id = associated;
    ...
    const contexts = { feedback };
    if (context.userAgent)
        contexts.browser = { name: context.userAgent };
    const device = {};
    if (context.screen)
        device.screen_resolution = context.screen;
    if (typeof context.online === "boolean")
        device.online = context.online;
    ...
    if (context.language)
        extra.language = context.language;
    if (context.timezone)
        extra.timezone = context.timezone;
    if (context.colorScheme)
        extra.color_scheme = context.colorScheme;
    if (context.connection)
        extra.connection = context.connection;
    if (ctx.screenshotUrl)
        extra.screenshot_url = ctx.screenshotUrl;
    const event = {
        timestamp,
        platform: "javascript",
    ...
    if (crumbs.length > 0)
        event.breadcrumbs = { values: crumbs.slice(-MAX_SENTRY_BREADCRUMBS) };
...
function screenshotBytes(report, ctx) {
    ...
    if (ctx.screenshot instanceof Uint8Array && ctx.screenshot.length > 0)
        return ctx.screenshot;
    const raw = (typeof report === "object" && report !== null ? report : {});
    if (typeof raw.screenshotDataUrl !== "string")
        return undefined;
    try {
 ...
 */
export function buildSentryEnvelope(report, options, ctx = {}) {
    ...
    const eventItemBytes = () => {
        const tags = event.tags;
        if (truncated.length > 0)
            tags.bugbottle_truncated = truncated.join(",");
        else
            delete tags.bugbottle_truncated;
        return encode(JSON.stringify(event));
    };
    const assemble = () => {
        const chunks = [header];
        chunks.push(...item({ type: options.itemType ?? "event", content_type: "application/json" }, eventItemBytes()));
    ...
    if ((body.length > maxEnvelopeBytes || eventItemBytes().length > MAX_SENTRY_EVENT_BYTES) &&
...
export class SentrySinkError extends SinkError {
    ...
    rateLimits;
    constructor(message, status, body, retryAfter, rateLimits) {
 ...
 * ```
 */
export function sentrySink(options) {
    ...
    return async (report, ctx = {}) => {
        ...
        if (!response.ok) {
            ...
            throw new SentrySinkError(messageFromBody(responseBody, fallback), response.status, responseBody, response.headers.get("Retry-After"), response.headers.get("X-Sentry-Rate-Limits"));
        }
    };
}
