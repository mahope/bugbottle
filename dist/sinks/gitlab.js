...
import { readBody, SinkError } from "./error.js";
 ...
 */
export function messageFromGitlabBody(body, fallback) {
    ...
    for (const key of ["message", "error", "error_description"]) {
        ...
        if (typeof value === "object" && value !== null) {
            const parts = [];
            for (const [field, reasons] of Object.entries(value)) {
 ...
 * ```
 */
export function gitlabSink(options) {
    ...
    return async (report, ctx = {}) => {
        ...
        if (options.labels && options.labels.length > 0)
            payload.labels = options.labels.join(",");
