 ...
 */
function trim(now) {
    while (groups.length > 1 && groups[1].start <= now - windowMs)
        drop();
    while (groups.length > 1 && totalBytes > maxBytes)
        drop();
}
function drop() {
    const gone = groups.shift();
    if (gone)
        totalBytes -= gone.bytes;
}
/**
 ...
 */
export function attachRrweb(record, options = {}) {
    if (attached)
        return resetRrweb;
    const seconds = typeof options.seconds === "number" && Number.isFinite(options.seconds) && options.seconds > 0
        ? options.seconds
    ...
    const emit = (event, isCheckout) => {
        ...
        if (isCheckout || groups.length === 0)
            groups.push({ start: timestamp, events: [], bytes: 0 });
...
export function getReplay() {
    if (totalBytes > maxBytes || totalBytes > MAX_REPLAY_BYTES)
        return null;
    const events = [];
...
export function isRrwebAttached() {
    return attached;
}
/**
