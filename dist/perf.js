 ...
 */
import { registerPerfSource, registerStorageSource } from "./registry.js";
import { MAX_COOKIE_NAMES, MAX_STORAGE_KEYS, MAX_STORAGE_KEY_LENGTH, MAX_STORAGE_VALUE_LENGTH, } from "./report-core.js";
 ...
 */
export function initPerf(options = {}) {
    if (initialised)
        return resetPerf;
    initialised = true;
    lcp = 0;
    ...
    if (snapshotStorage)
        registerStorageSource(getStorageSnapshot);
    return resetPerf;
}
...
export function getPerf() {
    ...
    if (memory && typeof memory.usedJSHeapSize === "number") {
        const mb = (bytes) => typeof bytes === "number" && Number.isFinite(bytes) ? Math.round(bytes / 1048576) : 0;
    ...
    try {
        ...
        if (typeof raw === "string" && raw) {
            ...
            for (const pair of raw.split(";")) {
                ...
                if (name)
                    cookies.push(name.slice(0, MAX_STORAGE_KEY_LENGTH));
            }
            if (cookies.length > 0)
                out.cookies = cookies;
        }
