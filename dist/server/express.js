...
function writeJson(res, status, body, cors) {
    res.status(status);
    if (cors) {
            ...
            if (signature &&
                ...
                if (signed || signature.require !== false) {
                    if (!warnedAboutParsedBody) {
                        ...
                        options.onError?.(new Error("bugbottle: expressHandler cannot verify a signature on a body that " +
