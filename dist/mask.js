 ...
 * says so.
 */
function collect(host, selector, out) {
    let children;
    try {
...
function bulletValue(el, undo) {
    const value = el.value;
    const placeholder = el.placeholder;
    if (!value && !placeholder)
        return;
    if (value)
        el.value = BULLET.repeat(value.length);
...
function paint(overlay, fill, box) {
 ...
 * back by the undo.
 */
function coverReplaced(el, overlay, fill) {
    const steps = [];
    ...
    if (!host || typeof el.getBoundingClientRect !== "function")
        return steps;
    const rect = el.getBoundingClientRect();
    const hostRect = host.getBoundingClientRect();
    const left = rect.left - hostRect.left + (host.scrollLeft || 0);
    const top = rect.top - hostRect.top + (host.scrollTop || 0);
    paint(overlay, fill, `left:${left}px;top:${top}px;width:${rect.width}px;height:${rect.height}px;`);
...
function cover(el, colour, undo) {
    ...
    if (REPLACED.test(el.tagName.toUpperCase())) {
        for (const step of coverReplaced(el, overlay, fill))
            undo.push(step);
        return;
    }
 ...
 */
export function applyMask(root, options = {}) {
    ...
    try {
        if (options.inputs ?? true) {
            ...
            for (const el of queryAll(root, INPUT_SELECTOR)) {
                ...
                if (el.tagName === "TEXTAREA")
                    bulletValue(field, undo);
                else if (el.tagName === "INPUT") {
                    if (!UNTYPED.test(field.type))
                        bulletValue(field, undo);
                }
        ...
        if (selector)
            for (const el of queryAll(root, selector))
                bulletText(el, undo);
        if (block) {
            for (const el of queryAll(root, block))
                cover(el, options.colour, undo);
        }
    }
    catch (err) {
