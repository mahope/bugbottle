...
// picture holds up on a dense screen. The panel itself is fixed to the corner.
await tab.setViewport({ width: 1010, height: 800, deviceScaleFactor: 2 });
...
/* Open it, say what went wrong, and point at something — the three things a
   reporter does. The message is the one from the hero paragraph, so the
   picture and the sentence above it are telling the same story. */
await tab.evaluate(async () => {
...
/* The wide picture: the panel at the right edge and about 380 points of the
...
await tab.screenshot({ path: join(site, "panel.png"), clip: wide });
...
await tab.screenshot({ path: join(site, "panel-narrow.png"), clip: narrow });
...
for (const [name, clip] of [
  ["panel.png", wide],
  ["panel-narrow.png", narrow],
]) {
  const { size } = await stat(join(site, name));
  // The hero is the first thing the page paints; both pictures are budgeted.
  console.log(
    `site/${name}: ${clip.width * 2}x${clip.height * 2} (width/height ${clip.width}/${clip.height}), ` +
